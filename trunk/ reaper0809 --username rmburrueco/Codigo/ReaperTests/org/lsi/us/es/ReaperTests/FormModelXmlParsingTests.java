...
{	
	...
	public void ParseXMLResults()
	{
		...
		Assert.assertTrue(f.getResults().get(0).getActions().get(0) instanceof Discard);
		
		
		Assert.assertTrue(f.getResults().get(1).getActions().get(0) instanceof Classify);
		Classify c=(Classify)f.getResults().get(1).getActions().get(0);
		Assert.assertEquals(c.getTag(),"testTagLabel");
		
		Assert.assertTrue(f.getResults().get(2).getActions().get(0) instanceof Store);
	}
	...
	public void ParseXMLReachFormMethod()
	{
		...
		Assert.assertEquals(((HttpGetRequest)f.getReachFormMethod()).getRequestUrl(), "http://google.com");
	}
